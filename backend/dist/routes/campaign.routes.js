...
router.get('/categories/stats', async (req, res) => {
    try {
        const categoryStats = await db
            ...
            .from(campaigns)
            .where(and(eq(campaigns.isActive, true), eq(campaigns.isApproved, true)))
...
router.get('/', optionalAuthMiddleware, async (req, res) => {
    try {
        const { page = '1', limit = '12', category, search, sortBy = 'recent', // recent, goal, raised
        featured, } = req.query;
        ...
        if (search) {
            conditions.push(sql `(${campaigns.title} ILIKE ${'%' + search + '%'} OR ${campaigns.summary} ILIKE ${'%' + search + '%'})`);
        }
        if (featured === 'true') {
...
router.get('/:identifier', optionalAuthMiddleware, async (req, res) => {
    try {
        ...
        const [campaign] = await db
            ...
            .from(campaigns)
            .innerJoin(users, eq(campaigns.userId, users.id))
            .where(and(isUuid ? eq(campaigns.id, identifier) : eq(campaigns.slug, identifier), eq(campaigns.isActive, true), eq(campaigns.isApproved, true)))
            .limit(1);
        if (!campaign) {
        ...
        const recentDonations = await db
            ...
            .from(donations)
            .where(and(eq(donations.campaignId, campaign.id), eq(donations.status, 'completed')))
            .orderBy(desc(donations.createdAt))
            .limit(10);
        // Get donation stats
        const [donationStats] = await db
            .select({
            totalDonors: sql `cast(count(distinct ${donations.donorEmail}) as int)`,
            totalDonations: sql `cast(count(*) as int)`,
        })
            .from(donations)
            .where(and(eq(donations.campaignId, campaign.id), eq(donations.status, 'completed')));
        res.json({
...
router.post('/', authMiddleware, requireVerifiedUser, async (req, res) => {
    try {
        const { title, summary, story, category, location, goalAmount, deadline, budgetBreakdown, coverImage, additionalMedia = [], } = req.body;
...
router.put('/:id', authMiddleware, requireVerifiedUser, async (req, res) => {
    try {
        ...
        if (budgetBreakdown !== undefined)
            updateData.budgetBreakdown = budgetBreakdown;
        if (coverImage !== undefined)
            updateData.coverImage = coverImage;
        if (additionalMedia !== undefined)
            updateData.additionalMedia = additionalMedia;
        // Update campaign
