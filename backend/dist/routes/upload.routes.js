...
router.post('/direct', authMiddleware, upload.single('file'), async (req, res) => {
    try {
        ...
        // Upload to S3
        const publicUrl = await S3Service.uploadFile(req.file.buffer, fileKey, req.file.mimetype);
        res.json({
            success: true,
            data: {
                fileKey,
...
router.post('/multiple', authMiddleware, upload.array('files', 10), async (req, res) => {
    try {
        ...
        const uploads = await Promise.all(files.map(async (file) => {
            ...
            const publicUrl = await S3Service.uploadFile(file.buffer, fileKey, file.mimetype);
            return {
                fileKey,
                publicUrl,
