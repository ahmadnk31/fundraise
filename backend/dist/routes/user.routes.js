...
router.get('/donations', authMiddleware, async (req, res) => {
    try {
        ...
        const [stats] = await db
            ...
            totalDonated: sql `cast(sum(${donations.amount}) as decimal)`,
        })
            .from(donations)
            .where(eq(donations.donorId, userId));
...
router.get('/dashboard', authMiddleware, async (req, res) => {
    try {
        ...
        const [campaignStats] = await db
            .select({
            totalCampaigns: sql `cast(count(*) as int)`,
            activeCampaigns: sql `cast(sum(case when ${campaigns.isActive} = true then 1 else 0 end) as int)`,
            totalRaised: sql `cast(sum(${campaigns.currentAmount}) as decimal)`,
        })
            .from(campaigns)
