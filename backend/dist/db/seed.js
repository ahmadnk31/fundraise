...
async function seed() {
    try {
        ...
        const hashedUsers = await Promise.all(seedData.users.map(async (user, index) => ({
            ...user,
        ...
        for (const campaign of insertedCampaigns) {
            ...
            for (let i = 0; i < donationCount && remainingAmount > 0; i++) {
                ...
                const donationAmount = Math.min(remainingAmount, Math.floor(Math.random() * (maxDonation - minDonation)) + minDonation);
                donationData.push({
